...
namespace IMS.Plugins.EFCoreSqlServer
{
	...
	{
		private readonly IDbContextFactory<IMSContext> dbContextFactory;
		...
		public async Task UpdateProductAsync(Product product)
		{
			...
			if (prod is not null)
			{
				...
				prod.ProductInventory = product.ProductInventory;
		...
		private void FlagInventoryUnchanged(Product product, IMSContext db)
		{
			if(product?.ProductInventory != null &&
				product.ProductInventory.Count > 0)
			{
				foreach(var prodInv in product.ProductInventory)
				{
